...
/* Solution Code */
void
Wakeup(struct thread *t, void *aux UNUSED)
{
...
thread_update_priority(struct thread *t)
{
  ...
  /* If the thread is holding locks, pick the one with the highest lock_priority.
  ...
  if (!list_empty(&t->locks_holding))
  {
	...
	lock_pri = list_entry(list_front(&t->locks_holding), struct lock, elem)->lock_priority;
    if (max_pri < lock_pri)
	  max_pri = lock_pri;
  }
  t->priority = max_pri;
